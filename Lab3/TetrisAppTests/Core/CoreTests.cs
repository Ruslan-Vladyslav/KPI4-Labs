...
namespace TetrisAppTests.Core
{
    public class CoreTests
    {
        ...
        [Fact]
        public void DropPiece_StopAtBottom()
        {
            var piece = new List<Point> { new Point(2, 0) };
            var screen = CreateScreen(5, 5, piece, new List<Point>());
            var finalScreen = _engine.DropPiece(screen);

            Assert.Single(finalScreen.Piece);
            Assert.Equal(4, finalScreen.Piece.First().Y);
        }

        [Fact]
        public void DropPiece_StopAbove()
        {
            var piece = new List<Point> { new Point(2, 0) };
        ...
        [Fact]
        public void DropPiece_StopAboveLandscape()
        {
            ...
            var landscape = new List<Point>
            {
                new Point(2, 5), new Point(3, 5), new Point(4, 5)
            };

            var screen = CreateScreen(10, 10, piece, landscape);
            var finalScreen = _engine.DropPiece(screen);
            var lowestPiecePoint = finalScreen.Piece.OrderByDescending(p => p.Y).First();

            Assert.Equal(4, lowestPiecePoint.Y);
            Assert.Contains(finalScreen.Piece, p => p.Y == 3);
        }
        #endregion
    }
}
